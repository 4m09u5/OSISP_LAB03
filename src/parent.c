...
enum Action {CREATE, KILL, LIST, KILL_ALL, SILENT, GARRULOUS, PRIORITIZE, EXIT, UNKNOWN_ACTION};
...
enum Action charToAction(char c) {
    ...
    case '-': return KILL;
    case 'l': return LIST;
    case 'k': return KILL_ALL;
    case 's': return SILENT;
    case 'g': return GARRULOUS;
    case 'p': return PRIORITIZE;
    case 'q': return EXIT;
...
void createChild() {
    ...
    if (!pid)
        execv("/home/dzmitry/Desktop/labs/OSISP/lab03/build/debug/child", (char* const*){NULL});
...
void handleAction(struct ParsedAction action) {
    ...
    case CREATE: createChild(); break;
    case KILL: killLatest(); break;
    case LIST: listAll(); break;
    case KILL_ALL: killAll(); break;
    case SILENT: silent(action.num); break;
    case GARRULOUS: garrulous(action.num); break;
    case PRIORITIZE: ask(action.num); break;
    case EXIT: exit(0);
    case UNKNOWN_ACTION: printf("Unknown action\n"); break;
    }
}
